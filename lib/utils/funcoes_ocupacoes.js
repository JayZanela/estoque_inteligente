...
export async function buscaOcupacoesEndereco(posicaoId) {
  const arrayRetorno = [];
  try {
    const runBuscaVariasOcupacoes = await buscarVariosDadosComFiltro(
      "ocupacoes_estoque",
      { posicao_id: posicaoId, quantidade: { gt: 0 } }
    );
...
export async function buscarOcupacoesProduto(produtoId) {
  const arrayRetorno = [];
  try {
    const runBuscaOcupsProdutos = await buscarVariosDadosComDetalhes(
      ...
      {
        posicoes_estoque_ocupacoes_estoque_posicao_idToposicoes_estoque: true,
      }
    );
