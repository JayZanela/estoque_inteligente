...
export default async function buscarSubCategoria(req, res) {
  ...
  const { id_categoria, nome_produto, descricao_produto } = req.body.parametro;

  const id_numero = parseInt(id_categoria);
  console.log("ID da categoria recebido:", id_numero);
  console.log(
    "Tipo de id_categoria:",
    typeof id_categoria,
    "Valor:",
    id_categoria
  );
  ...
  const listaFormatada = subcategorias
...

{
  ...
  if (pergunta_GPT.choices.length === 0) {
    ...
    return res
      .status(410)
      .json({
        error: "Erro ao processar a resposta da OpenAI, Busca de SubCategorias",
      });
  }

  const sub_categoria = JSON.parse(
    pergunta_GPT.choices[0].message.content.trim()
  );
